...
When you configure a volume, its UID is authoritative. Report Manager resolves the Craft volume for every storage operation and lets Craft apply the volume's configured subpath. If that volume or its filesystem is missing, invalid, read-only, throwing, or otherwise unavailable, the export fails with an actionable storage error. It never falls back to `exportPath` or another local directory, and a later operation retries the same UID so storage can recover without rewriting the setting.
...
Report Manager does not scan, move, copy, rewrite, or infer ownership for historical objects that were written outside a configured volume subpath. Until per-export storage affinity is available, an older export may appear unavailable when current canonical volume lookup cannot locate its object. Keep the historical object and export record in place rather than moving or deleting either based on a guessed location.
