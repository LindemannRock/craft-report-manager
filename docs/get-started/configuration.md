...
The Export settings warning uses the effective values after `config/report-manager.php` overrides, including an overridden volume UID or an override that selects the local path. It appears only on an ephemeral host when current behavior will use local storage. A configured volume UID is authoritative: if the volume or its filesystem is missing, invalid, read-only, throwing, or otherwise unavailable, Report Manager fails closed with a separate actionable error and never redirects the operation to `exportPath`, `@storage`, or another local directory. The setting remains unchanged, and later operations resolve the same UID again so a repaired volume can recover automatically.
...
All new volume operations use the Craft volume wrapper, so Craft applies the volume's configured subpath exactly once. Report Manager does not search for, relocate, copy, rewrite, or infer ownership of historical objects written outside that subpath. Until per-export storage affinity is available, older records can remain unavailable after storage changes or when their object exists only at a historical underlying-filesystem path; preserve those records and objects for an explicit compatibility decision.
