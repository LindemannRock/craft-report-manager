...
class ProcessScheduledReportsJob extends BaseJob implements RetryableJobInterface
{
     ...
     *
     * The worker should run often enough to catch any due report schedule.
     * Individual report schedules are calculated on the report records.
     */
    private function calculateNextRunDelay(): int
    {
        return $this->getSchedulerIntervalMinutes() * 60;
    }

    /**
     * Get the interval between scheduled report checks.
     */
    private function getSchedulerIntervalMinutes(): int
    {
        try {
            $config = Craft::$app->getConfig()->getConfigFromFile('report-manager');
        } catch (\Throwable) {
            $config = [];
        }

        $minutes = (int) ($config['schedulingInterval'] ?? 60);
